...
exports.validateDefaultProjectForFilesGlob =
...
function validateDefaultProjectForFilesGlob(options) {
  ...
  for (const glob of options.allowDefaultProjectForFiles) {
    if (glob === "*") {
      throw new Error(
        `allowDefaultProjectForFiles contains the overly wide '*'.${exports.DEFAULT_PROJECT_FILES_ERROR_EXPLANATION}`,
      );
    }
    if (glob.includes("**")) {
      throw new Error(
        `allowDefaultProjectForFiles glob '${glob}' contains a disallowed '**'.${exports.DEFAULT_PROJECT_FILES_ERROR_EXPLANATION}`,
      );
    }
  }
}
