...
module.exports = function slice(start, end) {
  ...
  if (!isArrayBuffer(O) || IsSharedArrayBuffer(O) || IsDetachedBuffer(O)) {
    throw new $TypeError(
      "receiver must be a non-detached, non-shared ArrayBuffer",
    ); // steps 2-4
  }
  ...
  if (
    ...
    throw new $TypeError(
      "Species constructor must produce a non-detached, non-shared Array Buffer",
    ); // steps 17-19
  }

  if (new$ === O) {
    throw new $TypeError(
      "new ArrayBuffer should not have been the same as the receiver",
    ); // step 20
  }

  if (arrayBufferByteLength(new$) < newLen) {
    throw new $TypeError(
      "new ArrayBuffer's byteLength must be at least the requested length",
    ); // step 21
  }
