...
exports.decodePDFRawStream = function (_a) {
  ...
  if (Filter instanceof PDFName_1.default) {
    ...
    for (var idx = 0, len = Filter.size(); idx < len; idx++) {
      stream = decodeStream(
        stream,
        Filter.lookup(idx, PDFName_1.default),
        DecodeParms && DecodeParms.lookupMaybe(idx, PDFDict_1.default),
      );
    }
  } else if (!!Filter) {
    throw new errors_1.UnexpectedObjectTypeError(
      [PDFName_1.default, PDFArray_1.default],
      Filter,
    );
  }
  return stream;
