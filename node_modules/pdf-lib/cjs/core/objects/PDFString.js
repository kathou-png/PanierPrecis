...
var PDFString = /** @class */ (function (_super) {
  ...
  PDFString.prototype.asBytes = function () {
    ...
    for (var idx = 0, len = this.value.length; idx < len; idx++) {
      ...
      if (!escaped) {
        if (byte === CharCodes_1.default.BackSlash) escaped = true;
        else pushByte(byte);
      } else {
        if (byte === CharCodes_1.default.Newline) pushByte();
        else if (byte === CharCodes_1.default.CarriageReturn) pushByte();
          ...
          byte >= CharCodes_1.default.Zero &&
          byte <= CharCodes_1.default.Seven
        ) {
          octal += char;
