...
export const DEFAULT_EXEC_ARGV =
  (SYNCKIT_EXEC_ARGV === null || SYNCKIT_EXEC_ARGV === void 0
    ? void 0
    : SYNCKIT_EXEC_ARGV.split(",")) || [];
...
export const isFile = (path) => {
  var _a;
  try {
    return !!((_a = fs.statSync(path, { throwIfNoEntry: false })) === null ||
    _a === void 0
      ? void 0
      : _a.isFile());
  } catch (_b) {
...
const setupTsRunner = (workerPath, { execArgv, tsRunner }) => {
  ...
  if (process.versions.pnp) {
    const nodeOptions =
      NODE_OPTIONS === null || NODE_OPTIONS === void 0
        ? void 0
        : NODE_OPTIONS.split(/\s+/);
    let pnpApiPath;
    try {
    ...
    if (
      pnpApiPath &&
      !(nodeOptions === null || nodeOptions === void 0
        ? void 0
        : nodeOptions.some(
            (option, index) =>
...
export const encodeImportModule = (moduleNameOrGlobalShim, type = "import") => {
  const { moduleName, globalName, named, conditional } =
...
export const _generateGlobals = (globalShims, type) =>
  globalShims.reduce(
...
function startWorkerThread(
  ...
  if (/\.[cm]ts$/.test(finalWorkerPath)) {
    ...
    if (!finalTsRunner) {
      throw new Error(
        "No ts runner specified, ts worker path is not supported",
      );
    } else if (
      [
  ...
  const worker = new Worker(
    (jsUseEsm && useGlobals) || (tsUseEsm && finalTsRunner === TsRunner.TsNode)
      ? dataUrl(
          `${generateGlobals(finalWorkerPath, finalGlobalShims)};import '${String(workerPathUrl)}'`,
        )
      : useEval
